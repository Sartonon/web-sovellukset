...
def index(req):
   ...
   # Haetaan kaikki omat treenit kannasta
   sql = """
   SELECT *
   From Treeni, Henkilo
   WHERE Treeni.Email = Henkilo.Email AND Henkilo.Email = :tunnus
   """
   ...
   pohja = os.path.join(os.path.dirname(req.filename), 'pohja.html')
   dom = parse(pohja)
   ...
   for rivi in cur:
     ...
     text = dom.createTextNode("Treeni " + str(i))
     i = i + 1
